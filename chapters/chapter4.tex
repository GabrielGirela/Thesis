...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=1\linewidth]{img/chapter4/vfc_vs_T_annoted.png}};
		\node[anchor=north east, xshift=-0pt, yshift=-50pt] at (img1.north east) {\textbf{(a)}};
	\end{tikzpicture}
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=1\linewidth]{img/chapter4/50_detail.png}};
...
%\subsection{Crystallinity and other secondary properties}
As it is in the nature of this work, as well as its main objective, to study the stability conditions for the formation and maintenance of a crystal mush inside the magma body, it is only natural to comment on the previously presented secondary properties. Crystallinity evolves in an interesting manner as it was presented in the precedent chapter (see figure \ref{fig:integrated_vf_c}). In the whole extent of the domain, crystallinity increases with time, as the global temperature decreases, following the natural and expected evolution. However, at the vicinity of the chamber walls, this phenomena does not occur. Instead, the crystallinity in these regions decreases as time advances. The mechanisms that produce this behavior are mainly two: remobilization and reabsorption. Remobilization is, as well, the effect of two main processes. The first one is remobilizaton by sinking of denser material (in the presented simulations, the denser material is batches of higher crystallinity and gassy magma mixture) from the bottom boundary down to the center and bottom of the chamber. This effect is shown in figure \ref{fig:73_76}, where it can be seen that batches of denser material are removed from the boundary as the time progresses. The second remobilization process is a result of the convective force, which drags material from the bottom boundary upwards. In figure \ref{fig:dp_rho_vel} it is clear how the convective patterns moving downwards, portrayed as the velocity vector arrows develop over-pressurized regions on the areas of inflexion of the convective cells. The combined effect of the drag force from the convective cell and the compression exerted by the over-pressure points is able to extrude the material and transport it upwards. The second mechanisms that inhibits crystallinity increase at the boundaries is reabsorption. Reabsorption can occur in two main scenarios. The first one, is remelting and re-dissolution due to local increases increases of heat, product of the release of the latent energy of phase change, which gives the cooling processes its characteristic stepped behavior. The same end can be achieved by the second mechanism, which is the previously described remobiliziation of the higher crystallinity and gassy batches. These are transported to the center of the magma chamber from the top or the bottom boundary areas, which remains at considerable higher temperatures, which equilibrates the phases, melting and dissolving them. 

\begin{figure}
	\centering
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=0.86\linewidth]{img/chapter4/vf_l_73.png}};
		\node[anchor=north east, xshift=-0pt, yshift=-50pt] at (img1.north east) {\textbf{(a)}};
	\end{tikzpicture}
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=0.86\linewidth]{img/chapter4/vf_l_74.png}};
		\node[anchor=north east, xshift=-0pt, yshift=-1pt] at (img1.north east) {\textbf{(b)}};
	\end{tikzpicture}
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=0.86\linewidth]{img/chapter4/vf_l_75.png}};
		\node[anchor=north east, xshift=-0pt, yshift=-50pt] at (img1.north east) {\textbf{(a)}};
	\end{tikzpicture}
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0)    {\includegraphics[width=0.86\linewidth]{img/chapter4/vf_l_76.png}};
		\node[anchor=north east, xshift=-0pt, yshift=-1pt] at (img1.north east) {\textbf{(b)}};
	\end{tikzpicture}	
...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=1\linewidth]{img/chapter4/dp_vel.png}};
		\node[anchor=north east, xshift=-0pt, yshift=-50pt] at (img1.north east) {\textbf{(a)}};
	\end{tikzpicture}
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0)    {\includegraphics[width=1\linewidth]{img/chapter4/vel_rho.png}};
		\node[anchor=north east, xshift=-0pt, yshift=-1pt] at (img1.north east) {\textbf{(b)}};
	\end{tikzpicture}	
	\caption{Snapshots of a zoomed-in area on the base simulation comparing the pressure difference ($dp$) on top with the bulk density of the system, in the bottom image. Sinking areas of higher density material develop depressurization in them, developing convection.}
	\label{fig:dp_rho_vel}
\end{figure}
