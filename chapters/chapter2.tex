...
% GOVERNING EQUATIONS
%--------------------------------------------------
In this chapter we present the physical and mathematical model. The governing equations for this problem are the weakly compressible Stokes flow equations for modeling the viscous flow and the heat transfer equation for computing the time dependent thermal evolution of the system. The presented mathematical model assumes an homogeneous mixture approach, and considers the mixture to be at thermodynamic, mechanical and chemical local equilibrium.  %Explain a bit each of the subsections
...
In our simulations, the magmatic properties are computed as a bulk quantities, meaning that the three coexisting phases contribute to the total value of a said property proportionally to its abundance within the magmatic system. Thus, each bulk property is computed as the weighted average of each phase's said property:

\begin{equation}
	\psi = \sum_i w_i\psi_i
\end{equation}
where $\psi$ is the property, $\psi_i$ and $w_i$ the said property and the mass fraction of the $i$th phase. Similarly, within each phase, properties are computed as the weighted average of its components.
\subsubsection{Velocity}
Velocity within magma chambers reflects the movement of the magma mixture and is a key indicator of the chamber’s dynamic state. It influences how heat, crystals, and volatiles are redistributed, and has a direct impact in processes like convection, crystallization, melt segregation, and volatile exsolution. In actively convecting chambers, higher velocities promote efficient thermal mixing, delaying cooling and solidification, and fostering more homogeneous magma compositions. Conversely, low or stagnant velocities may lead to stratification and the formation of stable crystal mushes or chemically zoned reservoirs.
Thicker chambers lead to higher Rayleigh numbers, which enhance buoyancy-driven convection. This increases vertical velocities, intensifies thermal gradients, and promotes efficient mixing. Thinner chambers, in contrast, and thanks to a reduced vertical extent, may exhibit sluggish convection or even purely conductive heat transfer, especially in more viscous magmas. Another important factor is the aspect ratio (height/width), which determines whether the flow is dominated by large-scale overturns, multi-cell convection, or stable stratification. %references
...
This relation depends upon pressure, temperature total silica in the system and water content dissolved in the melt. Similarly to the volatiles, the volume fraction ($\phi_c$) of crystals is computed using the following relation between the mass fraction, the total density ($\rho$) of the magma and the crystal phase density ($\rho_c$) is applied: 

\begin{equation}
	\phi_c = \frac{w_c\rho}{\rho_c}
\end{equation}
...
The density of the dissolved $H_2O$ and $CO_2$ is calculated from the model by \cite{burnham1969} as function of temperature and pressure:

\begin{equation}
	\begin{aligned}
		V_{mol} = a_0 + a_1 T + a_2 T^2 + a_3 T^3 + p(a_4 + a_5 T + a_6 T^2) \\
		+ p^2 (a_7 + a_8 T) + a_9 p^3, \\
		\rho_{dis} = M_{mol} / V_{mol}
	\end{aligned}
\end{equation}

where $V_{mol}$ is the molar volume of $H_20$ or $CO_2$ and $a_i$ are the fitting parameters. Density is then obtained by diving the molar mass by the molar volume of the volatiles mixture.
The density of the exsolved volatile phase, which includes water and carbon dioxide, is computed with the equation of estate from SOLWCAD (\cite{papale2006}) for consistency, and the density of the mineral phases is assumed constant and specific for each mineral species.
Since the homogeneous mixture approach is assumed, each mixture's density is given by the averaged density of the components that constitute it. First, the independent mixtures of gas, minerals and melt phase are computed by their local averages, and then the magma mixture's density is computed. Thus, for the total magma mixture, the density is computed by:

\begin{equation}
	\frac{1}{\rho} = \sum_i \frac{w_i}{\rho_i} = \sum_i^j \frac{w_i^j}{\rho_i^j}
\end{equation}

where $i$ determines the mixture components (e.g. $H_2O$ or $CO_2$ for the gas phase) and $j$ determines the phase (melt, gas of mineral). It is of the upmost importance, in order to fulfill the mass conservation criterion, that the following conditions must always be true:

\begin{equation}
	\sum_{i,j} w_i^j = \sum_i w_i = 1
\end{equation}
...
\subsubsection{Magma domain}
Glass cuttings from the IDDP-1 have been widely studied (\cite{elders2010},\cite{elders2011}, \cite{zierenberg2013}, \cite{masotta2018} and \cite{saubin2021} among others). The quenched magma is a subalkaline rhyolite of black to brown color, poorly vesiculated and sparsely phyric with some phenocrysts of titanomagnetite, plagioclase, augite and pigeonite. The composition, as listed in \cite{elders2014} in \ref{tab:rhyolite_composition}.
\begin{table}
	\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|c|}
		\hline
		SiO$_2$ & TiO$_2$ & Al$_2$O$_3$ & FeO$_\text{total}$ & MgO & CaO & Na$_2$O & K$_2$O & H$_2$O & CO$_2$ \\
		\hline
		76.53 & 0.32 & 12.01 & 2.73 & 0.18 & 1.32 & 3.78 & 3.06 & 2.00 & 0.352 \\
		\hline
	\end{tabular}
	\caption{Rhyolite melt composition (wt\%), including total H$_2$O and CO$_2$ as reported in \cite{elders2014}}
	\label{tab:rhyolite_composition}
\end{table}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=1\linewidth]{img/chapter2/sim_setup/volatiles_1.png}};
		\node[anchor=north east, xshift=-8pt, yshift=-3pt] at (img1.north east) {\textbf{(a)}};
	\end{tikzpicture}
	\begin{tikzpicture}
		\node[inner sep=0pt] (img1) at (0,0) {\includegraphics[width=1\linewidth]{img/chapter2/sim_setup/volatiles_2.png}};
...
\begin{table}[H]
	\resizebox{\columnwidth}{!}{%
		\begin{tabular}{|c|c|c|c|c|c|c|}
			...
			\hline
			Large & 2672 & 534 & $\sim 2 \times 10^{9}$ & 900 & 45 & 2, 0.352 \\
			\hline
		\end{tabular}%
	}
